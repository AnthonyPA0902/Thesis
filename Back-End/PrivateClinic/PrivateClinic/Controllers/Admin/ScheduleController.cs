...
namespace PrivateClinic.Controllers.Admin
{
	...
	public class ScheduleController : ControllerBase
	{
		...
		[HttpPost("schedule")]
		public IActionResult UpdateScheduleTable([FromBody] ExaminitionAppointment schedule)
		{
			if (schedule == null)
			{
			...
			else
			{
				...
				Console.WriteLine("Received checkup data:", schedule);
				...
				_dbContext.ExaminitionAppointments.Add(info);
		...
		{
			var tour = await _dbContext.ExaminitionAppointments.FirstAsync(ea => ea.Id == id);

			_dbContext.ExaminitionAppointments.Remove(tour);
			_dbContext.SaveChanges();
